...
```bash
>>> python -m src.actions.run_expr simple --n_jobs 5
```
"""
...
if __name__ == "__main__":
    ...
    data_list = [
        ...
        ("ES","biomass"), ("FPL","other"), ("ISNE","hydro"), ("NL","wind"), ("NYSIO","oil")
    ]
        ...
        if args.model_type == "simple":
            ...
            if args.n_jobs > 1:
                with mp.Pool(args.n_jobs) as pool:
                    best_performers = pool.starmap(name_and_train, [(config, GAN) for _ in range(5)])
                    best_performers = list(best_performers)
            else:
                best_performers = []
                for _ in range(5):
                    best_performers.append(GAN.train(config))
    ...
    expr_setup_dict = {"model_type": args.model_type, "n_datasets": len(data_list), "n_models": 5, "testing_sample_size": 1000}
